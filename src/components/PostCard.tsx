  ...
  if (viewMode === 'list') {
    return (
      <div 
        ...
        className={`group border-b border-border/25 last:border-b-0 py-3 px-4 transition-all hover:bg-muted/15 flex flex-col md:flex-row md:items-center justify-between gap-4 ${post.used ? 'bg-muted/5 opacity-75' : ''}`}
      >
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-1.5 mb-1.5">
            ...
            <Badge variant="outline" className={`border ${currentStatusProps.className} flex items-center gap-1 text-[9px] h-4.5 px-1.5 font-medium rounded-sm`}>
              <StatusIcon className="w-2.5 h-2.5" />
              {currentStatusProps.label}
            </Badge>
            {post.postType === 'thread' ? (
              <Badge variant="secondary" className="bg-purple-500/15 text-purple-600 dark:text-purple-400 border border-purple-500/10 flex items-center gap-1 text-[9px] h-4.5 px-1.5 font-semibold rounded-sm">
                <Layers className="w-2.5 h-2.5" /> Thread
              </Badge>
            ) : (
              <Badge variant="outline" className="text-muted-foreground/75 border-border/40 flex items-center gap-1 text-[9px] h-4.5 px-1.5 rounded-sm">
                <FileText className="w-2.5 h-2.5" /> Standard
              </Badge>
            )}
            {post.used && (
              <Badge variant="outline" className="bg-emerald-500/5 text-emerald-600 dark:text-emerald-400 border border-emerald-500/15 flex items-center gap-1 text-[9px] h-4.5 px-1.5 font-medium rounded-sm">
          ...
          <div className="flex items-center gap-4 mt-2 text-[9px] text-muted-foreground/75 font-mono">
            <span className="flex items-center gap-1 animate-fade-in">
              <Calendar className="w-3 h-3 text-muted-foreground/50" /> {formatDate(post.updatedAt)}
            </span>
    ...
    >
      ...
      <div className="flex items-center justify-between pb-2 mb-2.5 border-b border-border/20">
        <div className="flex items-center gap-1.5">
          ...
          <Badge variant="outline" className={`border ${currentStatusProps.className} flex items-center gap-1 text-[9px] h-4 px-1.5 font-medium rounded-sm`}>
            <StatusIcon className="w-2.5 h-2.5" />
            {currentStatusProps.label}
          </Badge>
        </div>
        <div>
          {post.postType === 'thread' ? (
            <Badge variant="secondary" className="bg-purple-500/15 text-purple-600 dark:text-purple-400 border border-purple-500/10 flex items-center gap-1 text-[9px] h-4 px-1.5 font-semibold rounded-sm">
              <Layers className="w-2.5 h-2.5" /> Thread
            </Badge>
          ) : (
            <Badge variant="outline" className="text-muted-foreground/70 border-border/40 flex items-center gap-1 text-[9px] h-4 px-1.5 rounded-sm">
              <FileText className="w-2.5 h-2.5" /> Standard
            </Badge>
          )}
        </div>
        ...
        <div className="w-full">
          <div className="flex justify-between items-center text-[9px] mb-1 font-mono tracking-tight text-muted-foreground/75">
            <span className="flex items-center gap-1">
              <Clock className="w-2.5 h-2.5 text-muted-foreground/45" /> {formatDate(post.updatedAt)}
            </span>
            <span className={charTextColor}>
              {charsLeft < 0 ? `Exceeded (+${Math.abs(charsLeft)})` : `${charCount}/${charLimit}`}
            </span>
          </div>
          <div className="w-full bg-muted/40 rounded-full h-[1.5px] overflow-hidden">
            <div 
            ...
            <Tooltip>
              <TooltipTrigger render={
                <Button 
                  variant="ghost" 
                  size="icon" 
                  className={`h-7 w-7 rounded-md transition-colors ${post.used ? 'text-emerald-500 bg-emerald-500/5 hover:bg-emerald-500/10' : 'text-muted-foreground/60 hover:text-foreground hover:bg-muted/70'}`}
                  onClick={toggleUsed}
                >
                  {post.used ? <CheckSquare className="h-3.5 w-3.5" /> : <Square className="h-3.5 w-3.5" />}
                </Button>
              } />
              <TooltipContent side="top" className="font-semibold text-xs py-1 px-2">
            ...
            <Tooltip>
              <TooltipTrigger render={
                <Button 
                  variant="ghost" 
                  size="icon" 
                  className={`h-7 w-7 rounded-md transition-all duration-200 ${copied ? 'text-emerald-500 bg-emerald-500/5' : 'text-muted-foreground hover:text-foreground hover:bg-muted/70'}`}
                  onClick={handleCopy}
                >
                  {copied ? <Check className="h-3.5 w-3.5 animate-scale-up" /> : <Copy className="h-3.5 w-3.5" />}
                </Button>
              } />
              <TooltipContent side="top" className="font-semibold text-xs py-1 px-2">
            ...
            <Tooltip>
              <TooltipTrigger render={
                <Button 
                  variant="ghost" 
                  size="icon" 
                  className="h-7 w-7 rounded-md text-muted-foreground hover:text-foreground hover:bg-muted/70"
                  onClick={() => {
                    setEditContent(post.content);
                    setEditStatus(post.status);
                    setEditType(post.postType);
              ...
              <Tooltip>
                <TooltipTrigger render={
                  <DropdownMenuTrigger render={
                    <Button 
                      variant="ghost" 
                      className="h-7 w-7 p-0 flex items-center justify-center rounded-md text-muted-foreground hover:text-foreground hover:bg-muted/70" 
                    />
                  }>
