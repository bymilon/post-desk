          ...
          <nav className="flex flex-col gap-3.5 flex-1 w-full px-2 items-center">
            ...
            <Tooltip>
              <TooltipTrigger 
                ...
                className={`w-9 h-9 rounded-md transition-all flex items-center justify-center cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500/45 ${
                  (activeView as string) === 'landing' 
                    ? 'bg-emerald-500/10 text-emerald-600 dark:bg-emerald-500/20 dark:text-emerald-400 hover:bg-emerald-500/15' 
                    : 'text-muted-foreground/85 hover:text-foreground hover:bg-muted/30'
                }`}
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden bg-background">
        ...
        <header className="h-12 flex-shrink-0 border-b border-border/20 flex items-center justify-between px-6 bg-card/45 backdrop-blur-md z-15 sticky top-0 transition-all duration-350">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h1 className="text-xs font-bold tracking-tight text-foreground flex items-center gap-1.5 font-sans font-sans">
                {activeView === 'posts' ? 'Posts Space' : activeView === 'inspirations' ? 'Inspirations Board' : 'Marketing Identity'}
